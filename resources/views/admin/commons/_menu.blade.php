    ...
    <div class="sidebar-collapse">
        <ul class="nav" id="side-menu">
            <li class="nav-header text-center">
                <div class="dropdown profile-element">
                    ...
                    <ul class="dropdown-menu animated fadeInRight m-t-xs">
                        <li><a class="J_menuItem" href="{{route('admins.edit', $admin->id)}}">修改头像</a></li>
                        <li><a class="J_menuItem" href="{{route('admins.edit', $admin->id)}}">修改密码</a></li>
                        <li class="divider"></li>
            ...
            @foreach(Auth::guard('admin')->user()->getMenus() as $key => $rule)
                @if($rule['route'] == 'index.index')
                    <li>
                        <a title="{{$rule['name']}}" href="{{route($rule['route'])}}">
                            <i class="fa fa-{{$rule['fonts']??'desktop'}}"></i>
                ...
                @else
                    <li>
                        <a title="{{$rule['name']}}"> <i class="fa fa-{{$rule['fonts']??'desktop'}}"></i>
                            <span class="nav-label">{{$rule['name']}}</span>
