...
final class Tree
{
    ...
    static public function channelLevel(
        ...
        foreach ($data as $v) {
            if ($v[$fieldPid] == $pid) {
                ...
                $arr[$v[$fieldPri]]["_data"] =
                    self::channelLevel($data, $v[$fieldPri], $html, $fieldPri,
                        $fieldPid, $level + 1);
            }
        }
    ...
    static public function channelList(
        ...
        $data = self::_channelList($data, $pid, $html, $fieldPri, $fieldPid,
            $level);
        if (empty($data)) {
            return $data;
        }
    ...
    static private function _channelList(
        ...
        foreach ($data as $v) {
            ...
            if ($v[$fieldPid] == $pid) {
                ...
                array_push($arr, $v);
                $tmp =
                    self::_channelList($data, $id, $html, $fieldPri, $fieldPid,
                        $level + 1);
     ...
     * @return array
     */
    static public function parentChannel(
        $data,
        $sid,
        $fieldPri = 'cid',
        $fieldPid = 'pid'
        ...
        if (empty($data)) {
            ...
            foreach ($data as $v) {
                if ($v[$fieldPri] == $sid) {
                    $arr[] = $v;
                    $_n = self::parentChannel($data, $v[$fieldPid], $fieldPri,
                        $fieldPid);
                    if (!empty($_n)) {
        ...
        foreach ($data as $key => $item) {
            if ($item['parent_id'] == $parent_id) {
                foreach ($data as $k => $value) {
                    if ($value['parent_id']
                        == $item['id']
                    ) {  //如果子级权限的parend_id等于顶级权限的id就说明这个权限是顶级权限的子级权限
